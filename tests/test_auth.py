...
class TestRegister:
    def test_success(self, client):
        res = client.post('/snap/auth/register', json={
    ...
    def test_email_normalised_to_lowercase(self, client):
        ...
        # Auto-verify so the login check doesn't block us
        user = db.session.execute(
            select(User).where(User.email == 'upper@example.com')
        ).scalar_one()
        user.verified_at = utcnow()
        db.session.commit()

        # Should be able to log in with lowercase version
        login = client.post('/snap/auth/login', json={
            'email': 'upper@example.com',
            'password': 'strongpassword1',
        })
    ...
    def test_missing_email_returns_422(self, client):
        res = client.post('/snap/auth/register', json={'password': 'strongpassword1'})
        assert res.status_code == 422

    def test_missing_password_returns_422(self, client):
        res = client.post('/snap/auth/register', json={'email': 'missing@example.com'})
        assert res.status_code == 422
    ...
    def test_non_json_body_returns_422(self, client):
        res = client.post('/snap/auth/register', data='not json', content_type='text/plain')
        assert res.status_code == 422
...
class TestLogin:
    def test_success_returns_tokens(self, client, registered_user):
        res = client.post('/snap/auth/login', json=registered_user)
    ...
    def test_wrong_password_returns_401(self, client, registered_user):
        res = client.post('/snap/auth/login', json={
            'email': VALID_EMAIL,
    ...
    def test_missing_fields_returns_422(self, client):
        res = client.post('/snap/auth/login', json={'email': VALID_EMAIL})
        assert res.status_code == 422
...
class TestRefresh:
    def test_success_returns_new_access_token(self, client, auth_tokens, app):
        res = client.post('/snap/auth/refresh', json={
    ...
    def test_invalid_token_returns_401(self, client):
        res = client.post('/snap/auth/refresh', json={'refresh_token': 'notavalidtoken'})
        assert res.status_code == 401
    ...
    def test_revoked_token_returns_401(self, client, auth_tokens):
        ...
        client.post('/snap/auth/logout', json={'refresh_token': auth_tokens['refresh_token']})
...
class TestLogout:
    def test_success_returns_204(self, client, auth_tokens):
        res = client.post('/snap/auth/logout', json={
            'refresh_token': auth_tokens['refresh_token'],
        })
    ...
    def test_logout_is_idempotent(self, client, auth_tokens):
        ...
        client.post('/snap/auth/logout', json={'refresh_token': auth_tokens['refresh_token']})
        res = client.post('/snap/auth/logout', json={'refresh_token': auth_tokens['refresh_token']})
        assert res.status_code == 204

    def test_logout_with_garbage_token_returns_204(self, client):
        res = client.post('/snap/auth/logout', json={'refresh_token': 'garbagetoken'})
        assert res.status_code == 204
